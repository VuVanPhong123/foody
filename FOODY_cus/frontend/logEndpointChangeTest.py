...
class LogEndpointChangeTest(Screen):
    BASE_ENDPOINT = "http://localhost:8011/login"
    ...
    def __init__(self, **kwargs):
        ...
        self.login_btn = RoundedButton(text="Đăng nhập (endpoint động)",
                                       size_hint=(.5, .07),
    ...
    def start_login(self, *_):
        ...
        # This creates an invalid endpoint that doesn't exist on the backend
        endpoint = f"http://localhost:8011/login_{random.randint(1000,9999)}"
        try:
            r = requests.post(endpoint, json={"username": uname, "password": pw}, timeout=5)
            msg = "Đăng nhập thành công!" if r.status_code == 200 else r.json().get("detail", "Sai hoặc endpoint không tồn tại")
