...
from backend.ordermanager import OrderManager
...
class Makingorders(Screen):
    def __init__(self, top_manager=None, **kwargs):
        ...
        self.menu_items = self.order_manager.menu  
    ...
    def add_to_order(self, instance):
        item = instance.text
        quantity = 1
        self.order_manager.add_to_order(self.selected_items, item, quantity)

    def complete_order(self, instance):
        if any(qty > 0 for qty in self.selected_items.values()):
            total_price = self.order_manager.calculate_total(self.selected_items)
    ...
    def complete_revenue(self, done_orders, total_price, row_box):
        ...
        self.order_manager.save_order_v2(done_orders, total_price)
